...
@app_blue.route('/add_course/', methods=['POST'])
def add_crouse():
    """
    添加课程
    """
    ...
    for k in course_dict:
        course = Course()
        course.c_name = k
        course.c_desc = course_dict[k]
        db.session.add(course)
    db.session.commit()
    return '创建课程成功'


@app_blue.route('/stu_add_course/<int:id>/', methods=['GET', 'POST'])
...
@app_blue.route('/sel_course/<int:id>/', methods=['GET'])
def sel_course(id):
    stu = Student.query.get(id)
    courses = stu.course
...
@app_blue.route('/del_course/<int:id>/', methods=['GET'])
def del_course(id):
    c_id = request.args.get('c_id')
    course = Course.query.get(c_id)
    stu = Student.query.get(id)
    # list删除 remove(), del list[0], pop()
